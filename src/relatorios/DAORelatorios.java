...
public class DAORelatorios extends ConexaoMySql{
    ...
    public boolean gerarRelatorioFuncionario() {
        try {
              this.conectar();
              this.executarSQL("SELECT * FROM funcionarios;");
              JRResultSetDataSource jrRS = new JRResultSetDataSource(getResultSet());
            // caminho do arquivo dentro dos pacotes  
            InputStream caminhoRelatorio = this.getClass().getClassLoader().getResourceAsStream("ArquivosJasper/relatorioFuncionarios.jasper");
            JasperPrint jasperPrint = JasperFillManager.fillReport(caminhoRelatorio, new HashMap(), jrRS);
